...
# MAGIC %md
# MAGIC # Data Engineering for Ground Truth Table
# MAGIC
# MAGIC ## Notebook Purpose
# MAGIC This notebook focuses on performing data engineering tasks on the ground truth table, including reading data, creating/updating tables, and registering the table using MLCORE_SDK.
# MAGIC
...
# MAGIC We read the ground truth DataFrame from the DBFS path and ensure that the Hive database is created if it doesn't already exist. The Hive database is crucial for managing structured data.
# MAGIC
# MAGIC #### 3. Table Creation/Update
# MAGIC In this section, we create or update the ground truth table within the Hive database. This table stores the processed data, making it accessible for further analysis and usage.
# MAGIC
...
# MAGIC We register the ground truth table using the `log` function from the MLCORE_SDK. This step is essential for tracking and managing the table's lifecycle. We use the `operation_type = "register_table"` parameter to specify the registration action.
# MAGIC
# MAGIC #### Data Set
...
# MAGIC - The notebook follows a structured approach: data loading, ground truth table creation, and table registration.
...
# MAGIC %md
# MAGIC ### Data Engineering Configurations
# MAGIC
# MAGIC This notebook focuses on data engineering tasks, emphasizing the extraction of essential parameters and configurations for data processing.
# MAGIC
# MAGIC #### General Parameters
# MAGIC
# MAGIC We start by loading the following general parameters from the SolutionConfig YAML file:
# MAGIC
# MAGIC - `sdk_session_id`: This parameter is dynamically generated using the `env` variable.
...
# MAGIC
# MAGIC In this section, we configure parameters that are specific to data engineering:
# MAGIC
# MAGIC - `primary_keys`: These represent the primary keys for the feature table, crucial for data management and integrity.
# MAGIC - `ground_truth_table_name`: The name of the ground truth table, where accurate reference data is stored.
# MAGIC - `ground_truth_dbfs_path`: The DBFS (Databricks File System) path to the source ground truth data that requires processing.
# MAGIC
# MAGIC ##### This structured data engineering notebook streamlines the process of configuring and executing data engineering tasks. It aims to simplify data management and processing within the Databricks environment, with a focus on both feature data and ground truth data.
...
# DBTITLE 1,Load the ground_truth_df from the path
...
ground_truth_df = ground_truth_df.drop('date','timestamp')
